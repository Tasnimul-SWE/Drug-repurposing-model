...
.calculateES <-
function(data,type=c("fixed","dynamic","range"),adj=c("BH","qvalue"),lengthtest=250,ranges=seq(100,2000,by=100),pvalues=NULL,dynamic.fdr=0.01,signif.fdr=0.01,customRefDB=NULL,case=c("disease","drug"),noperm=1000,stat=c("KS","WSR")){
  message(".calculateES Started")
  case=match.arg(case)
	type=match.arg(type)
...
function(ESscores,rankdata=NULL,rankdatainv=NULL,uplength=250,downlength=250,noperm=1000,fdr=0.01,drugPRLs,adj=adj,stat=c("KS","WSR")){
  message(".significantES Started")
	...
	return(resultslist)
message(".significantES Done")	
}
.findSignifCompounds <-
function(scores,rankdata=NULL,rankdatainv=NULL,noperm=1000,uplength=250,downlength=250,fdr=fdr,adj=adj,stat=c("KS","WSR")){
  message(".findSignifCompounds Started")  
...
pvaladj<-emppvals  # added by azam   adjustment for later!
	
# 	sel<-which(pvaladj<fdr)    # commented out by azam
# 	signifcompounds<-scores[sel] # commented out by azam

# names(signifcompounds)<-names(scores)[sel] # commented out by azam

signifcompounds<-pvaladj  # added by azam
names(signifcompounds)<-names(scores)  # added by azam

message(".findSignifCompounds Done")  
...
function(refmatrix,inputmatrix,lengthtest=250){
  ...
  message(".KS.drug.disease Started")
	refmatrix<-as.matrix(refmatrix)
	inputmatrix<-as.matrix(inputmatrix)
	N<-lengthtest
	Nh<-nrow(refmatrix)
